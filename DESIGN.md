...
| `--accent-500` | `oklch(0.74 0.140 78)`  | `#D89B3C` | **o ponto**: botão primário, o "+" do cabeçalho, marcação |
...
- **Celular (< 768px):** conteúdo em coluna única, navegação inferior fixa com cinco destinos
  (só ícones), ação primária da tela como "+" no cabeçalho — com bandeja quando há mais de uma
  ação, direto quando há uma só. Detalhes e formulários abrem em folha inferior arrastável. Os
  dois editores (`/fichas/[id]`, `/pedidos/[id]`) não têm navegação inferior: o cabeçalho tem
  o voltar, o aparelho volta por gesto, e o resumo de custo desce até o pé (`#d152`).
...
distintivo). Superfícies opacas: sem vidro, sem brilho, sem plástico. Sombras baixas
...
| **Botão mais e bandeja**            | só no celular; a ação primária da tela é o "+" no cabeçalho: só o traço em `--accent-500` sobre a tinta, sem círculo, alvo de 44px, `aria-label`. Com mais de uma ação, `aria-haspopup="dialog"` e abre a folha inferior (o `Painel`) com uma linha por ação: ícone + nome, alvo de 52px, lista com divisórias; a primeira é a de criar. Opção sem rede fica `aria-disabled` com a razão embaixo do nome. Com uma ação só, age no toque e o `aria-label` é o nome dela (`#d151`). Não existe mais pílula flutuante. |
...
| **Painel lateral / folha inferior** | mesmo componente e mesma API; 260ms. Modal só para confirmação destrutiva: título com a consequência, dois botões, sem "X" — `Confirmacao`, o `<dialog>` nativo.                                                                                                                                                                                                                                                                                                                                                    |
...
| **Navegação inferior**              | cinco destinos, só o ícone de 24px, alvo de 56px; o rótulo vai em `aria-label` e `title` (`#d150`). Ativo em `--accent-ink` com pílula `--brand-100` de 32×56 atrás do ícone: é ela que diz "você está aqui". **Não** `--accent-500`: sobre a superfície ele reprova AA.                                                                                                                                                                                                                                            |
...
- **Um botão primário por tela.** No celular ele é o "+" do cabeçalho ou o botão do painel de
  pé; no desktop, o canto superior direito do cabeçalho. Enquanto o estado vazio ensina a tela,
  a ação primária é dele: o botão do cabeçalho e o "+" saem. A tela Hoje é de leitura; a
